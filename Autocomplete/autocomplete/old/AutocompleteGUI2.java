 ...
 *
 *
 *  BUG: Dropdown menu slightly wider and misaligned with search box.
 *
 *  BUG: Search when term has an & seems to be truncated.
 *       Hannah Montana & Miley Cyrus
 *  
 *  BUG: Checking the checkbox, typing exactly one character, and checking
 *       the box again inverts its behaviour from what is expected sometimes.
 *       Haven't done enough testing to pinpoint when/why this happens.
 *       [ replaced with ActionListener; does this fix the bug? ]
 *  
 *  BUG: Cursor does not return to original position when focus leaves and
 *       returns to input field.
 *  
 *
 *  TODO: Replace the "magic numbers" (two appearances of 1000) with sensible
 *        constants from the GUI element properties. --- Lines 338-339
 *  
 *  
 ...
 *
 *  FEATURE: option to ignore diacritics. The easiest way to do this is to
 *           change Term to accept Comparables instead of Strings, whereby
 *           one can write a Comparable implementation that takes String
 *           arguments and whose compareTo calls Collator.compare(). The
 *           private class DiacriticInsensitveString has already been added
 *           below. It cannot be used, however, unless Term is changed as
 *           mentioned above.
 *  
 *
...
//            suggestions.setFixedCellHeight(40);
            ...
            suggestionsPanel.setPreferredSize(new Dimension(searchText.getPreferredSize().width+2*prefBarWidth, 1000));
            ...
            this.setPreferredSize(new Dimension(searchText.getPreferredSize().width+2*prefBarWidth, this.getPreferredSize().height));
            this.setMaximumSize(new Dimension(searchText.getMaximumSize().width+2*prefBarWidth, searchText.getMaximumSize().height+suggestionsPanel.getMaximumSize().height));
         ...
         private void updateListSize()
         {
            ...
            suggestions.setPreferredSize(new Dimension(searchText.getPreferredSize().width, rows*suggestions.getFixedCellHeight()));
         }
            ...
            if (text.equals("")) {
               ...
               if (Math.min(k, allResults.length) > 0) {
                  for (int i = 0; i < results.length; i++) {
                     ...
                     if (tab < 0) {
                         throw new RuntimeException("allMatches() returned an array with an entry without a tab: '" + next + "'");
                     }
                     ...
                     results[i] = "<html><table width=\"" + searchText.getPreferredSize().width + "\">"
                                + "<tr><td align=left>" + query.substring(0, textLen) + "<b>" + query.substring(textLen) + "</b>";
                     if (displayWeights) {
                         results[i] += "<td width=\"10%\" align=right><font size=-1><span id=\"weight\" style=\"float:right;color:gray\">" + weight + "</font>";
                     }
